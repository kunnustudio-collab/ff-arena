...
router.get('/', (req, res) => {
    ...
    const list = tournament_service_1.TournamentService.getAllTournaments({
        mode: mode,
        map: map,
        status: status,
...
router.get('/my/matches', auth_middleware_1.authenticate, (req, res) => {
    const userId = req.user.id;
    const userRegs = Array.from(data_store_1.store.registrations.values())
        .filter((r) => r.user_id === userId)
        .sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
    const matches = userRegs.map((reg) => {
        const tournament = data_store_1.store.tournaments.get(reg.tournament_id);
        const result = Array.from(data_store_1.store.results.values()).find((r) => r.tournament_id === reg.tournament_id && r.user_id === userId);
        ...
        if (tournament) {
            ...
            if (isTimePassed || tournament.status === 'LIVE') {
                const matchRoom = Array.from(data_store_1.store.matches.values()).find((m) => m.tournament_id === tournament.id);
                if (matchRoom) {
                    canViewRoom = true;
...
router.get('/:id', (req, res, next) => {
    try {
        const tournament = tournament_service_1.TournamentService.getTournamentById(req.params.id);
...
router.post('/:id/join', auth_middleware_1.authenticate, async (req, res, next) => {
    try {
        ...
        const registration = await tournament_service_1.TournamentService.joinTournament(req.user.id, req.params.id, {
            inGameName,
            inGameUid,
            teamName,
        });
...
router.get('/:id/room', auth_middleware_1.authenticate, (req, res, next) => {
    try {
        const creds = tournament_service_1.TournamentService.getRoomCredentials(req.user.id, req.params.id);
...
router.post('/:id/result', auth_middleware_1.authenticate, async (req, res, next) => {
    try {
        ...
        const result = await tournament_service_1.TournamentService.submitMatchResult(req.user.id, req.params.id, {
            kills: Number(kills),
...
router.post('/', (0, admin_middleware_1.requireAdmin)(['SUPER_ADMIN', 'ADMIN', 'TOURNAMENT_ADMIN']), (req, res) => {
    ...
    data_store_1.store.tournaments.set(id, newTournament);
    // Create match room
    const matchId = (0, uuid_1.v4)();
    data_store_1.store.matches.set(matchId, {
        id: matchId,
        tournament_id: id,
        match_number: 1,
        status: 'SCHEDULED',
...
router.put('/:id/room', (0, admin_middleware_1.requireAdmin)(['SUPER_ADMIN', 'ADMIN', 'TOURNAMENT_ADMIN']), (req, res) => {
    const { roomId, roomPassword, status } = req.body;
    const match = Array.from(data_store_1.store.matches.values()).find((m) => m.tournament_id === req.params.id);
    if (match) {
        if (roomId)
            match.room_id = roomId;
        if (roomPassword)
            match.room_password = roomPassword;
        if (status)
            match.status = status;
...
router.post('/:id/disburse', (0, admin_middleware_1.requireAdmin)(['SUPER_ADMIN', 'ADMIN', 'TOURNAMENT_ADMIN', 'FINANCE_ADMIN']), async (req, res, next) => {
    try {
        const { approvedResults } = req.body;
        const result = await tournament_service_1.TournamentService.verifyAndDisbursePrizes(req.user.id, req.params.id, approvedResults);
        res.json(result);
    }
    catch (err) {
        next(err);
...
router.post('/:id/cancel', (0, admin_middleware_1.requireAdmin)(['SUPER_ADMIN', 'ADMIN']), async (req, res, next) => {
    try {
        const { reason } = req.body;
        const result = await tournament_service_1.TournamentService.cancelAndRefundTournament(req.user.id, req.params.id, reason || 'Organizer cancellation');
        res.json(result);
    }
    catch (err) {
        next(err);

...
router.use((0, admin_middleware_1.requireAdmin)(['SUPER_ADMIN', 'ADMIN', 'FINANCE_ADMIN', 'TOURNAMENT_ADMIN', 'KYC_ADMIN', 'SUPPORT_AGENT']));
// 1. Dashboard Overview Metrics & Charts
router.get('/dashboard', (req, res) => {
    const totalUsers = Array.from(data_store_1.store.users.values()).filter((u) => u.role === 'PLAYER').length;
    const verifiedKycCount = Array.from(data_store_1.store.kycProfiles.values()).filter((k) => k.status === 'VERIFIED').length;
    const pendingKycCount = Array.from(data_store_1.store.kycProfiles.values()).filter((k) => k.status === 'PENDING').length;
    const activeTournaments = Array.from(data_store_1.store.tournaments.values()).filter((t) => t.status === 'OPEN' || t.status === 'LIVE' || t.status === 'FULL').length;
    const pendingWithdrawalsCount = Array.from(data_store_1.store.withdrawals.values()).filter((w) => w.status === 'UNDER_REVIEW' || w.status === 'REQUESTED').length;
    const openDisputesCount = Array.from(data_store_1.store.disputes.values()).filter((d) => d.status === 'OPEN').length;
    const flaggedRiskCount = Array.from(data_store_1.store.riskEvents.values()).filter((r) => !r.is_reviewed).length;
    let totalEntryRevenue = 0;
    for (const reg of data_store_1.store.registrations.values()) {
        if (reg.status === 'CONFIRMED')
            totalEntryRevenue += reg.entry_fee_paid;
    }
    let totalPrizePaid = 0;
    for (const entry of data_store_1.store.ledger) {
        if (entry.category === 'PRIZE')
            totalPrizePaid += entry.amount;
    }
    res.json({
        success: true,
        data: {
            metrics: {
                ...
                activeTournaments,
                todayRegistrations: data_store_1.store.registrations.size,
...
router.get('/users', (req, res) => {
    const usersList = Array.from(data_store_1.store.users.values()).map((u) => {
        const profile = data_store_1.store.profiles.get(u.id);
        const wallet = data_store_1.store.wallets.get(u.id);
        const kyc = data_store_1.store.kycProfiles.get(u.id);
        return {
            id: u.id,
            phone: u.phone,
            email: u.email,
...
router.put('/users/:id/status', (req, res) => {
    const user = data_store_1.store.users.get(req.params.id);
    if (!user)
...
router.put('/users/:id/wallet-freeze', (req, res) => {
    const wallet = data_store_1.store.wallets.get(req.params.id);
    if (!wallet)
        throw api_error_1.ApiError.notFound('Wallet not found');
...
router.get('/kyc/queue', (req, res) => {
    const queue = Array.from(data_store_1.store.kycProfiles.values()).map((k) => {
        const user = data_store_1.store.users.get(k.user_id);
        const profile = data_store_1.store.profiles.get(k.user_id);
        return {
            ...k,
            phone: user?.phone,
            email: user?.email,
...
router.get('/withdrawals', (req, res) => {
    const list = Array.from(data_store_1.store.withdrawals.values()).map((w) => {
        const profile = data_store_1.store.profiles.get(w.user_id);
        const user = data_store_1.store.users.get(w.user_id);
        return {
            ...w,
            username: profile?.username,
            phone: user?.phone,
...
router.put('/withdrawals/:id', (req, res) => {
    const withdrawal = data_store_1.store.withdrawals.get(req.params.id);
    if (!withdrawal)
        throw api_error_1.ApiError.notFound('Withdrawal record not found');
    ...
    if (status === 'REJECTED') {
        const wallet = data_store_1.store.wallets.get(withdrawal.user_id);
        if (wallet) {
...
router.get('/antifraud/events', (req, res) => {
    res.json({ success: true, data: data_store_1.store.riskEvents });
});
router.put('/antifraud/events/:id', (req, res) => {
    const event = data_store_1.store.riskEvents.find((e) => e.id === req.params.id);
    if (!event)
        throw api_error_1.ApiError.notFound('Risk event not found');
...
router.get('/audit-logs', (req, res) => {
    res.json({ success: true, data: data_store_1.store.auditLogs.slice(0, 100) });
});
// 7. Disputes Management
router.get('/disputes', (req, res) => {
    res.json({ success: true, data: Array.from(data_store_1.store.disputes.values()) });
});
router.put('/disputes/:id', (req, res) => {
    const dispute = data_store_1.store.disputes.get(req.params.id);
    if (!dispute)
        throw api_error_1.ApiError.notFound('Dispute not found');
...
router.get('/reports/:type', (req, res) => {
    ...
    if (reportType === 'users') {
        ...
        for (const l of data_store_1.store.ledger) {

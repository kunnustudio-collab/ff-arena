...
router.get('/tickets', auth_middleware_1.authenticate, (req, res) => {
    const userId = req.user.id;
    const tickets = Array.from(data_store_1.store.supportTickets.values())
        .filter((t) => t.user_id === userId)
        .sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
    res.json({ success: true, data: tickets });
});
// POST /api/v1/support/tickets
router.post('/tickets', auth_middleware_1.authenticate, (req, res, next) => {
    try {
        ...
        data_store_1.store.supportTickets.set(ticketId, newTicket);
...
router.post('/tickets/:id/reply', auth_middleware_1.authenticate, (req, res, next) => {
    try {
        const ticket = data_store_1.store.supportTickets.get(req.params.id);
        if (!ticket)
            throw api_error_1.ApiError.notFound('Support ticket not found');
...
router.post('/disputes', auth_middleware_1.authenticate, (req, res, next) => {
    try {
        ...
        data_store_1.store.disputes.set(disputeId, dispute);
